...
export class PluginSelectorComponent extends OverlayPanel {
	...
	constructor(
		...
		const items: SelectItem[] = plugins.map(({ plugin, marketplace, scope }) => {
			...
			const status = installed ? ` [${t("plugin.selector.installedTag")}]` : "";
		...
		if (items.length === 0) {
			items.push({
				value: "__empty__",
				label: t("plugin.selector.empty"),
				description:
					marketplaceCount === 0 ? t("plugin.selector.emptyNoMarketplace") : t("plugin.selector.emptyNoPlugins"),
			});
		}
